...
function drawIntroScreen() {
  ...
  text(introText, width/2, height/2 - 80, width * 0.75, 260);
...
function checkGestures() {
  ...
  if (predictions.length > 0 && millis() > cooldown) {
    ...
    let directionThreshold = max(30, palmSize * 0.4);
    let fistThreshold = max(35, palmSize * 0.45);

    // 是否握拳
    let fistScore = 0;
    fistScore += dist(indexTip[0], indexTip[1], palmCenter[0], palmCenter[1]);
    fistScore += dist(middleTip[0], middleTip[1], palmCenter[0], palmCenter[1]);
    fistScore += dist(ringTip[0], ringTip[1], palmCenter[0], palmCenter[1]);
    fistScore += dist(pinkyTip[0], pinkyTip[1], palmCenter[0], palmCenter[1]);
    let isFist = fistScore / 4 < fistThreshold;

    // 手指指向判斷：必須明確伸直
    let indexLength = dist(indexTip[0], indexTip[1], indexPIP[0], indexPIP[1]);
    let mappedDir = -1;
    if (!isFist && indexLength > directionThreshold) {
